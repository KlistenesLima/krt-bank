...
public class AccountService : IAccountService
{
    ...
    // Stubs — serão implementados futuramente
    ...
    public Task<Result<BalanceResponse>> GetBalanceAsync(Guid id, CancellationToken cancellationToken)
        => Task.FromResult(Result.Fail<BalanceResponse>("Not implemented"));
    ...
    public Task<Result> BlockAsync(Guid id, string reason, CancellationToken cancellationToken)
        => Task.FromResult(Result.Ok());
    
    public Task<Result> UnblockAsync(Guid id, CancellationToken cancellationToken)
        => Task.FromResult(Result.Ok());
    
    public Task<Result> CloseAsync(Guid id, string reason, CancellationToken cancellationToken)
        => Task.FromResult(Result.Ok());
    
    public Task<Result> UpdateAsync(UpdateAccountRequest request, CancellationToken cancellationToken)
        => Task.FromResult(Result.Ok());
    
    public Task<Result<TransactionResponse>> DebitAsync(DebitAccountRequest request, CancellationToken cancellationToken)
        => Task.FromResult(Result.Fail<TransactionResponse>("Not implemented"));
